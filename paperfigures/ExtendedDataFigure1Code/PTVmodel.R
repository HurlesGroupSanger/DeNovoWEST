...
powpath = "/nfs/ddd0/jk18/dne/DeNovoWEST/input/extended_denovoWEST_results.tab"
#import libraries-------------------
library(reshape2)
library(ggplot2)
library(data.table)
...
for(i in 1:length(prophi_vals)){
  for(j in 1:length(elof_vals)){
    store_divergence_prob[j,i] <- mean(replicate(50,SimModel(prophi_vals[i],elof_vals[j],res,phi = T,nsim = 20,edist = F, esd = esd)))
  }
}
end_time <- Sys.time()
end_time - start_time
