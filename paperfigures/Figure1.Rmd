...
pdf("./plots/fig1a.pdf",width = 7.5,height = 7)
...
pval_5bins_gene_category_diagnostic_lists <- res_mup %>%
  filter(sig==TRUE) %>%
  mutate(pvalbin = ifelse(diag_group=='consensus', case_when(denovoWEST_p_full < 2.5e-2 & denovoWEST_p_full >2.5e-5 ~ '2.5e-2 to 2.5e-5',
                             denovoWEST_p_full < 2.5e-5 & denovoWEST_p_full >2.5e-8 ~ '2.5e-5 to 2.5e-8',
                             denovoWEST_p_full < 2.5e-8 & denovoWEST_p_full >2.5e-11 ~ '2.5e-8 to 2.5e-11',
                             denovoWEST_p_full < 2.5e-11 & denovoWEST_p_full >2.5e-14 ~ '2.5e-11 to 2.5e-14',
                             denovoWEST_p_full < 2.5e-14 ~ '< 2.5e-14'), case_when(denovoWEST_p_ud < 2.5e-2 & denovoWEST_p_ud >2.5e-5 ~ '2.5e-2 to 2.5e-5',
                             denovoWEST_p_ud < 2.5e-5 & denovoWEST_p_ud >2.5e-8 ~ '2.5e-5 to 2.5e-8',
                             denovoWEST_p_ud < 2.5e-8 & denovoWEST_p_ud >2.5e-11 ~ '2.5e-8 to 2.5e-11',
                             denovoWEST_p_ud < 2.5e-11 & denovoWEST_p_ud >2.5e-14 ~ '2.5e-11 to 2.5e-14',
                             denovoWEST_p_ud < 2.5e-14 ~ '< 2.5e-14'))) %>%
  group_by(pvalbin, diag_group) %>%
  tally()

pval_5bins_gene_category_diagnostic_lists$pvalbin <- factor(pval_5bins_gene_category_diagnostic_lists$pvalbin, levels=c('2.5e-2 to 2.5e-5','2.5e-5 to 2.5e-8','2.5e-8 to 2.5e-11','2.5e-11 to 2.5e-14', '< 2.5e-14'))
...
pdf("./plots/fig1b.pdf",width = 8,height = 6)
ggplot(pval_5bins_gene_category_diagnostic_lists, aes(x=pvalbin, y=n, fill=diag_group)) +
  ...
  theme(axis.text.x  = element_text(size=14), axis.text.y = element_text(size=14), axis.title = element_text(size=16), legend.title = element_blank(), legend.text = element_text(size=14), axis.line = element_blank(), axis.ticks.x = element_blank()) +
  scale_y_continuous('Number of genes',breaks=seq(0,100,25), labels=seq(0,100,25)) +
  scale_x_discrete('p-value bins', labels=c('2.5e-2 to\n2.5e-5', '2.5e-5 to\n2.5e-8', '2.5e-8 to\n2.5e-11', '2.5e-11 to\n2.5e-14', '< 2.5e-14'))
dev.off()
```
...
novel <- res[res$diag_group == "novel",]
...
pdf("./plots/fig1d.pdf",width = 5,height = 5)
...
pdf("./plots/fig1e.pdf",width = 5,height = 5)
