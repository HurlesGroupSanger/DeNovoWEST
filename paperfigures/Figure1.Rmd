...
#pdf("fig1a.pdf",width = 9,height = 8)
...
gofScale <- scale_colour_manual(name = "Most significant in:",values = novcol)
...
#pdf("fig1c.pdf",width = 9,height = 8)
ggplot(novel,aes(x=missense_variant,y = lofcount)) +
  geom_point(alpha = 0.5,aes(size = lg10p,color = gofsig)) + theme_classic()+
  geom_label_repel(aes(label = symbol,color = gofsig),box.padding   = 0.25,point.padding = 0.5,size =2.75,show.legend = F) +
  xlab("Number of missense DNMs") + ylab("Number of PTV DNMs")+ 
  labs(colour = "Most sig in:",size = "-log10(p-value)") +
