...
ddg2p_file = '../input/DDG2P_21_9_2018.csv'
```
...
pdf("./plots/fig3a.pdf",width = 6,height = 5.5)
...
pdf("./plots/fig3b.pdf",width = 6,height = 5.5)
p
dev.off()
```
...
# plot
# bp <- barplot(obscounts$x/expcounts$x,ylim = c(0,60),names.arg = c("low","medium","high"),xlab = "Genes' likelihood of ultrasound abnormality",ylab = "PTV enrichment",col = traffic,border = NA,cex.axis = 1.2,cex.lab = 1.4,cex.names = 1.2)
# arrows(bp[,1],ci.lower,bp[,1],ci.upper,length = 0,lwd = 1.5)

# plot
pdf("./plots/fig3c.pdf",width = 5,height = 5)
...
```{r, fig.align='center'}
pdf("./plots/fig3d.pdf",width = 7,height = 5)
...
plot_3c <- ggplot(abnormal_us_obs_exp, aes(x=Group, y=observed/expected)) +
  geom_bar(stat='identity', fill=traffic) +
  theme_classic() +
  ylim(c(0,60)) +
  labs(y = 'PTV enrichment') +
...
plot_3d <- ggplot(ptv_syn_gnomad, aes(x = lofbin, y = ptv_syn)) +
  geom_point(size=5) +
  theme_classic() +
  ylim(c(0,0.021)) +
  geom_errorbar(aes(ymin=lower_bound, ymax=upper_bound), width=.1) +
...
pdf("./plots/combined_fig3.pdf",width = 12, height = 10)
top_row <- plot_grid(plot_3a, plot_3b, labels=c('A', 'B'), scale=0.95)
bottom_row <- plot_grid(plot_3c, plot_3d, nrow = 1, labels = c('C', 'D'), rel_widths = c(1, 1.5), scale=0.95)
